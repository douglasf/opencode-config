...
3. **Mark related tasks as done** — For each task that is clearly resolved by the commit:
    - First, use `vault0_task-view` to check if the task already has a solution field (the execute agent typically writes solution notes during implementation).
    - If the task **already has a solution**: use `vault0_task-complete` and **append** the commit reference to the existing solution — do NOT overwrite what the execute agent wrote. Example: `solution: "<existing solution text>\n\nCommitted in <hash>: <commit message summary>"`
...
- **Do NOT overwrite existing solution notes** — the execute agent writes detailed solution notes during implementation. Your job is to append the commit reference, not replace the execute agent's work.
