...
This config uses a **multi-agent architecture**. When you type in OpenCode, you're talking to the **orchestrator** (`orchestrate`) — it never does work itself. Instead, it delegates to specialized agents: **execute** for implementation (reads, writes, searches, runs commands), **investigate** for deep analysis (read-only codebase investigation), and **git** for version control (commits, pushes, PRs — only via slash commands). The orchestrator routes your request to the right agent, waits for results, and synthesizes a response.

You can also switch to the **plan** agent (via tab) when you want to think through a feature or design before building. The plan agent interviews you about what you want to build, delegates investigation and planning to the **architect** (who uses **investigate** for deep analysis), and presents concise metadata summaries. You iterate with the plan agent on scope and decisions — then hand off to the execute agent for implementation when ready.

### Day-to-Day Workflow

- **Ask a question or request a change** — just type naturally. The orchestrator will route to the right agent. Implementation requests go to the execute agent, who investigates and executes autonomously. Analysis-only requests go to the investigate agent.
...
- **Planning** — switch to the **plan** tab when you need to think through something bigger: a new feature, architectural change, or multi-step refactor. Describe what you want and the plan agent will guide you through a structured planning process, producing a plan document you can implement later.
- **Code changes** — describe what you want changed and where. The execute agent will read the relevant files, make edits, and run tests if appropriate. Review the changes before committing.
...
- The execute agent reports back what it did but does not commit. You always control when changes are committed.
- If you're in strict mode and the execute agent says it can't run a command, that's the guardrails working. Either switch to YODO mode or run the command yourself.
...
~/.config/opencode/
  ...
  agent/                  # Agent definitions (orchestrate, execute, investigate, plan, architect, git, quick-answer)
...
└──────────┬────────────────────────┬─────────────────────────────┘
  ...
  "do" │       │ "know"         │       │ (hand off to
       ...
       ▼       ▼                ▼       ▼
    ┌──────┐ ┌───────────┐  ┌──────────┐ ┌───────┐
    │execute│ │investigate│  │architect │ │execute│
    │      │ │           │  │          │ │       │
    └──┬───┘ └───────────┘  └────┬─────┘ └───┬──┘
       │      (terminal)         │            │
       │                         ▼            │
...
**Routing rules:**
- **orchestrate** routes based on intent: "do something" → execute, "know something" → investigate. Default bias is execute.
- **execute** can self-delegate to investigate mid-task when deep investigation is needed.
- **plan** delegates to architect for investigation + planning, or to execute for implementation handoff.
- **architect** delegates to investigate for deep codebase research.
- **investigate**, **git**, and **quick-answer** are terminal — they do not delegate further.

### Agent Roles

| Agent | Role | Model | Mode | Can Delegate To |
|---|---|---|---|---|
| **orchestrate** | Orchestrator — receives user requests, routes to the right agent. Never reads code, edits files, or runs commands (except read-only git for context). | Claude Haiku 4.5 | Primary | execute, investigate |
| **execute** | Executor — reads, writes, edits, searches, and runs commands. Does all implementation work. Owns the full cycle from investigation to delivery. | Claude Opus 4.6 | Subagent | investigate |
| **investigate** | Investigator — deep read-only codebase analysis. Traces dependencies, maps architecture, returns structured findings with file paths and line numbers. Never modifies anything. | Claude Opus 4.6 | Subagent | *None* |
| **architect** | Planner — combines investigation (directly + via investigate) with structured planning. Creates vault0 task hierarchies. Returns only metadata summaries to caller. | Claude Opus 4.6 | Subagent | investigate |
| **plan** | Planning coordinator — interviews users about what to build, delegates investigation and planning to the architect. Only sees plan metadata, never source code. | Claude Haiku 4.5 | Primary | architect, execute |
...
|---|---|---|---|---|---|---|---|
| **orchestrate** | Limited (prompt context only) | No | No | Yes (status, log) | No | execute, investigate | No |
| **execute** | Full | Full | Extensive allow-list (read-only + builds/tests) | Yes | No | investigate | Yes |
| **investigate** | Full | No | Read-only exploration only | Yes | No | No | Yes |
| **architect** | Full | vault0 tasks only | Minimal (ls, cat, git read) | Yes | No | investigate | Yes |
| **plan** | vault0 tasks only | No | Git context only | Yes | No | architect, execute | No |
| **git** | Full | No | Git + GitHub CLI (full write) | Yes | Yes | No | No |
| **quick-answer** | No | No | No | No | No | No | Yes |

**Key constraints:**
- **Git write operations are forbidden** for all agents except git. The execute, investigate, architect, and plan agents cannot run `git add`, `git commit`, `git push`, etc.
- **The orchestrator never does work.** The orchestrate agent has no grep, glob, write, or edit tools. It only delegates via `Task()`.
- **The investigate agent is strictly read-only.** No file writes, no builds, no package installs, no code execution. Not even `make` or `npm run`.
- **The execute agent has broad bash access** in strict mode, but it's an explicit allow-list — default-deny with whitelisted commands for builds, tests, linting, and read-only tools. Dangerous operations (cloud mutations, remote access, package publishing, system commands) are denied.
- **The architect agent can only create vault0 tasks** — it cannot modify source code.
- **The plan agent cannot read source code** — its access is restricted to vault0 task metadata only.
...
| **Bash for execute** | Default-deny, explicit allow-list (builds, tests, read-only tools) | Default-ask, broad allow-list |
| **File editing** | Allowed (execute agent has full write/edit) | Allowed |
