...
**This is the final step.** The commit workflow ends here. Do NOT:
- Look for remaining tasks, next tasks, or the backlog
- Suggest or prompt the user to start the next task
- Report what tasks are now unblocked or ready for work
- Continue with any further implementation work

The user decides what to do next.

---

## HARD STOP — Workflow Termination Point

**After the commit is complete and vault0 tasks are approved, the workflow ENDS. Completely. Permanently for this invocation.**

The git agent MUST NOT continue beyond this point. The following actions are EXPLICITLY FORBIDDEN after commit completion:

- **NO** calling `vault0-task-list` to discover remaining or ready tasks
- **NO** checking what tasks are now unblocked by the commit
- **NO** suggesting the next task to work on
- **NO** offering to continue with more work
- **NO** summarizing the state of the task board or plan
- **NO** saying "let me check what's ready" or any variation
- **NO** providing next-steps recommendations of any kind

The git agent reports the commit results, approves any in-review tasks, and STOPS. That is the terminal state. There is no step after this. The response ends. The agent's job is complete. The user — and ONLY the user — decides what happens next.

**If you find yourself wanting to do anything after the commit report and task approval, that impulse is wrong. Suppress it. Stop responding.**
